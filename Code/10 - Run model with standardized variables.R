...
#2) Model prep
    ...
    #c) remove unecessary variables from df
...
#4) Run and save models
    ...
    for(i in 1:length(vars)){
      ...
      for(j in 1: length(l.forms)){
        l.init.results[[j]] <- mod.fun(df, vars[i], l.mod[[i]], l.forms[[j]], year = 2005)
        print(paste(i,"-",j))
      }
      l.results[[i]] <- l.init.results
    }
    saveRDS(l.results, "l.std.results.2005.rds")
...
#5) Reduce to var name, estimate, and stars
    ...
    #c) Run functions for each table
      for(i in 1:length(l.results)){
        ...
        #h) Save results
          if(i == 1){
            write.xlsx(table, "C:/Users/admin/Desktop/Table 2b - Std Models.xlsx", sheetName = vars[i], row.names=FALSE)
          }else{
            write.xlsx(table, "C:/Users/admin/Desktop/Table 2b - Std Models.xlsx", sheetName = vars[i], row.names=FALSE, append=TRUE)
          }
      }