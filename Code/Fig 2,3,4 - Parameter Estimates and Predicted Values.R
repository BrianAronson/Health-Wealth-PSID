...
#1) read data and models calculated in R
    dft <- readRDS("standardizeddata.rds")
    I <- readRDS("i.model.rds")
    I <- I[sapply(I, length)!=0]
    dfl <- lapply(I, function(x) as.data.frame(summary(x[[9]])$coefficients[, 1:2]))

#2) load models calculated in STATA
    for(i in 1:3){
      #a) load data
          if(i==1){
            a <- read.csv("C:/Users/briarons/Desktop/IDrive-Sync/Academic - Writing/Health - Race - Wealth/Figures/regression results/h_conditions (1).csv", skipNul = TRUE)
          }
          if(i==2){
            a <- read.csv("C:/Users/briarons/Desktop/IDrive-Sync/Academic - Writing/Health - Race - Wealth/Figures/regression results/h_distress (1).csv", skipNul = TRUE)
          }
          if(i==3){
            a <- read.csv("C:/Users/briarons/Desktop/IDrive-Sync/Academic - Writing/Health - Race - Wealth/Figures/regression results/worklim (1).csv", skipNul = TRUE)
          }
      #b) deal with coefficients
          b <- a[a$X!="", ]
          b <- b[-1, ]
          b <- b[-(match("Observations", b[, 1]):nrow(b)), ]
          b <- sapply(b, as.character)
          b <- gsub("[/*]", "", b)
          b <- gsub("[/b]", "", b)
          b <- as.data.frame(b, stringsAsFactors=F)
          b[2:ncol(b)] <- sapply(b[2:ncol(b)], as.numeric)
          b1 <- b
      #c) deal with standard errors
          b <- a[a$X=="", ]
          b <- b[!apply(b, 1, function(x) all(x=="")), ]
          b <- sapply(b, function(x) as.character(x))
          b <- gsub("[/)]", "", b)
          b <- gsub("[/(]", "", b)
          b <- as.data.frame(b, stringsAsFactors=F)
          b <- sapply(b, function(x) as.numeric(x))
          b2 <- b
      #d) select columns
          b3 <- cbind(b1[, c(1, 10)], b2[, 10])
      #e) change text labels to match R ordering
          mdf <- data.frame(V1= c("(Intercept)", "ego_black2", "ego_age", "ego_age2", "ego_female", "fam_region", "year2", "died", "log_income", "ego_black2:log_income", "ego_dg_highschool", "ego_dg_somecollege", "ego_dg_bachelors", "ego_dg_advanced", "eq_home_d", "peq_home", "peq_debt", "peq_stock", "peq_savings", "ego_black2:ego_age", "ego_black2:ego_age2", "ego_black2:eq_home_d", "ego_black2:peq_home", "ego_black2:peq_debt", "ego_black2:peq_stock", "ego_black2:peq_savings", "ego_black2:ego_dg_highschool", "ego_black2:ego_dg_somecollege", "ego_black2:ego_dg_bachelors", "ego_black2:ego_dg_advanced", "log_wealth", "ego_black2:log_wealth"), 
                          V2= c("Constant", "1.ego_lack2", "ego_age", "ego_age2", "ego_female", "1.fam_region", "year2", "1.died", "log_income", "1.ego_lack2#c.log_income", "ego_dg_highschool", "ego_dg_somecollege", "ego_dg_achelors", "ego_dg_advanced", "eq_home_d", "peq_home", "peq_det", "peq_stock", "peq_savings", "1.ego_lack2#c.ego_age", "1.ego_lack2#c.ego_age2", "1.ego_lack2#c.eq_home_d", "1.ego_lack2#c.peq_home", "1.ego_lack2#c.peq_det", "1.ego_lack2#c.peq_stock", "1.ego_lack2#c.peq_savings", "1.ego_lack2#1.ego_dg_highschool", "1.ego_lack2#1.ego_dg_somecollege", "1.ego_lack2#1.ego_dg_achelors", "1.ego_lack2#1.ego_dg_advanced", "log_wealth", "1.ego_lack2#c.log_wealth"))
          b3 <- b3[match(mdf$V2, b3[, 1]), ]
          b3[, 1] <- mdf$V1
          b3 <- b3[!is.na(b3[, 2]), ]
          row.names(b3) <- b3[, 1]
          b3 <- b3[, -1]
          names(b3) <- c("Estimate", "Std. Error")
          dfl[[5+i]] <- b3
    }
         
#3) create a dataframe consisting of key IVs, DV name, parameter estimate, SEs
    #a) append variables to dataframe
        df <- do.call(rbind, dfl)
    #b) add var name and remove row name
        df$Var <- unlist(sapply(dfl, row.names))
        row.names(df) <- NULL
    #c) add DV name
        DV2 <- c("Self-Rated Health", "BMI", "Heart Attack", "Stroke", "Disability", "# of Conditions", "Distress", "Work Limitations")
        df$DV <- c(rep("Self-Rated Health", 3), rep(DV2, each=(nrow(df)/8)))
    #d) reorder
        DV1 <- c("Self-Rated Health", "BMI", "# of Conditions", "Heart Attack", "Stroke", "Work Limitations", "Disability", "Distress")
        DV12 <- c(rep("Self-Rated Health", 3), rep(DV1, each=(nrow(df)/8)))
        DV123 <- match(DV12, df$DV)
        DV1234 <- duplicated(DV123)
        for(i in 1:length(DV123)){
          if(DV1234[i]){
            DV123[i] <- DV123[i-1]+1
          }
        }
        df <- df[DV123, ]
    #e) create upper and lower bound estimates
        df$CIlower <- df$Estimate-df$`Std. Error`*1.96
        df$CIupper <- df$Estimate+df$`Std. Error`*1.96
    #f) indicate if significant
        df$significant <- ifelse(sign(df$CIlower)==sign(df$CIupper), "black", "grey")
    
#4) create a temporary df of variables I care about
    dft <- df[df$Var %in% c("eq_home_d", "peq_home", "peq_debt", "peq_stock", "peq_savings"), ]
    #set order of variables
        dft$Var <- factor(rep(c("Own Home", "Home Equity", "Debt", "Stocks", "Savings"), 8), levels=c("Debt", "Stocks", "Savings", "Home Equity", "Own Home"), ordered = T)
...
#7) kill heart attack; stroke; BMI
    dft <- dft[!(dft$DV %in% c("Heart Attack", "Stroke", "BMI")), ]

#8) set limits for each y axis
    dft  <-  data.table(dft)
    dft[, y_min := -max(c(abs(CIupper), abs(CIlower))), by = DV]
    dft$y_min <- ceiling(dft$y_min*22)/20
    dft$y_max <- (-dft$y_min)
