...
#2) Model prep
    ...
    #b) set models
        l.mod <- list()
        l.mod[[1]] <-  function(form, df) glmmadmb(formula = form, data = df, zeroInflation=TRUE, family="nbinom", extra.args="-ndi 1000000")
...
#4) Run and save models
    ...
    for(j in 1: length(l.forms)){
      l.results[[j]] <- mod.fun(df, "h_distress", l.mod[[j]], form5b, year = 2005)
      print(j)
    }
