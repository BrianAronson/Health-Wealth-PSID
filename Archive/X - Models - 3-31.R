...
#4 - run models
    ...
    for(i in 1:length(results)){
      ...
      for(j in 1:length(tempres)){
        ...
        if(j==1){
          write.xlsx(tempres2, file=tempvar, sheetName=tempname, row.names=FALSE)
        }else{
          write.xlsx(tempres2, file=tempvar, sheetName=tempname, append=TRUE, row.names=FALSE)
        }
      }
    }
