...
#3 - Model prep
    ...
    #c - models to test
        form1<-y~ego_black2 + ego_age+ego_age2 + ego_female + ego_black2 *(ego_age+ego_age2)+fam_region +year2+just_died+already_died  #+ (1 | ind_id) + (1 | fam_id_68)
...
#4 - Run models
    ...
    for(i in 1:length(vars)){
      ...
      #b - Run models
        ...
        m2<-model(form2,df=I3)
        m3<-model(form2b,df=I3)
        m4<-model(form3,df=I3)
        m5<-model(form3b,df=I3)
        m6<-model(form4,df=I3)
        m7<-model(form4b,df=I3)
        m8<-model(form5,df=I3)
        m9<-model(form5b,df=I3)
...
#6 - save results to excel
    for(i in 1:length(results)){
      #determine file name/location
          ...
          # fname<-"C:/Users/bda13/Desktop/10-12 Mixed Effects 2005-2011.xlsx"
