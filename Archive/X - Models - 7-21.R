...
#2 - Create a few other variables
    ...
    #Correct variable order
        ...
        corder<-c("ego_black2","ego_age","ego_age2","ego_female","ego_black2:ego_age","ego_black2:ego_age2","year2","just_died","already_died","log_wealth","ego_black2:log_wealth","peq_debt","peq_stock","peq_savings","peq_home","ego_black2:peq_debt","ego_black2:peq_stock","ego_black2:peq_savings","ego_black2:peq_home","log_income","ego_dg_highschool","ego_dg_somecollege","ego_dg_bachelors","ego_dg_advanced","(Intercept)")
...
#3 - Model prep
    ...
    #b - create model output function to make results pretty
        ...
        #fixed effects model
            model1<-function(x,df){suppressWarnings(prstars(as.data.frame(summary(plm(formula=x,data=df, index=c("ind_id", "year"), model="within"))$coefficients)[c("Estimate","Pr(>|t|)")]))}
    ...
    #c - models to test
        form1<-y~ego_black2 + ego_age+ego_age2 + ego_female + ego_black2 *(ego_age+ego_age2) +year2+just_died+already_died + (1 | ind_id) + (1 | fam_id_68)
        form2a<-update(form1,~. + log_wealth)
        form2b<-update(form1,~. + log_wealth*ego_black2)
        form3a<-update(form1,~. + log_wealth + peq_home+peq_debt+peq_stock+peq_savings)
        form3b<-update(form1,~. + log_wealth + ego_black2*(peq_home+peq_debt+peq_stock+peq_savings))
        form4<-update(form1,~. + log_wealth + ego_black2*(peq_home+peq_debt+peq_stock+peq_savings) + log_income + ego_dg_highschool+ego_dg_somecollege+ego_dg_bachelors+ego_dg_advanced)
...
#4 - Run models
    ...
    for(i in 1:length(vars)){
      #a - Select variable
        I3$y<-I3[,vars[i]]
      #b - Run models
        m1<-model(form1,df=I3)
        m2<-model(form2a,df=I3)
        m3<-model(form2b,df=I3)
        m4<-model(form3a,df=I3)
        m5<-model(form3b,df=I3)
        m6<-model(form4,df=I3)
      ...
      #d - prep empty df to put results into
        df<-data.frame(var=unique(c(row.names(m1),row.names(m2),row.names(m3),row.names(m4),row.names(m5),row.names(m6))))
      #e - bind vars to df
...
#6 - save results to excel
    for(i in 1:length(results)){
      #determine file name/location
          fname<-"C:/Users/bda13/Desktop/7-30 Mixed Effects all Data.xlsx"
          # fname<-"C:/Users/admin/Desktop/7-30 Mixed Effects all Data.xlsx"
      #grab results
          tresult<-results[[i]]
